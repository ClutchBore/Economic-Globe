...
export default function AIComparison({ countryCodeA, countryCodeB }) {
  ...
  return (
    <div className="flex flex-col gap-2.5 rounded-none border border-slate-200 bg-slate-50 px-4 py-[15px]">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[11.5px] font-bold tracking-wider text-slate-500">AI COMPARISON</span>
        </div>
      ...
      {state.status === 'ready' && <p className="text-[13.5px] leading-[1.55] text-slate-700">{state.comparison}</p>}
    </div>
  )
}
