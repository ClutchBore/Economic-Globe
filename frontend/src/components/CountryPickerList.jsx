...
export default function CountryPickerList({ excludeCodes = [], onSelect }) {
  ...
  return (
    <div className="flex max-h-[220px] flex-col gap-1 overflow-y-auto rounded-none border border-white/[0.1] bg-slate-800/60 p-1.5">
      {options.map((c) => (
        <button
          key={c.country_code}
          onClick={() => onSelect(c)}
          className="flex items-center justify-between rounded-none px-3 py-2 text-left hover:bg-white/[0.06]"
        >
